...
final class PlatformDependent0 {
    ...
    private static final ILogger logger = Logger.getLogger(PlatformDependent0.class);
    ...
    static {
        ...
        if (logger.isFinestEnabled()) {
            logger.finest("java.nio.ByteBuffer.cleaner: "+ (cleanerField != null? "available" : "unavailable"));
        }
        ...
        if (logger.isFinestEnabled()) {
            logger.finest("java.nio.Buffer.address: "+ (addressField != null? "available" : "unavailable") );
        }
        ...
        if (addressField != null && cleanerField != null) {
            try {
                ...
                if (logger.isFinestEnabled()) {
                    logger.finest("sun.misc.Unsafe.theUnsafe: " + (unsafe != null? "available" : "unavailable") );
                }
                ...
                try {
                    ...
                    logger.finest("sun.misc.Unsafe.copyMemory: available");
                } catch (NoSuchMethodError t) {
                    logger.finest("sun.misc.Unsafe.copyMemory: unavailable");
                    throw t;
        ...
        if (unsafe == null) {
            ...
            UNALIGNED = unaligned;
            if (logger.isFinestEnabled()) {
                logger.finest("java.nio.Bits.unaligned: "+ UNALIGNED);
            }
        }
    }
